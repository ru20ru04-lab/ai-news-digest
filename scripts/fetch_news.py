...
【マークアップ記法（重要）】
すべてのテキストフィールドで以下の記法を使えます。これらは UI 側で装飾されます。
- **太字**：重要なキーワード・固有名詞・サービス名・人名・組織名を `**...**` で囲む（太字で表示）
- ==下線強調==：特に注目してほしい数値・キーワード・要点を `==...==` で囲む（teal 系の下線で表示）
- !!注意・リスク!!：注意点・リスク・問題点・規制違反・セキュリティ懸念・著作権侵害・差別・倫理的問題・誤情報リスクなど、
  読者が**警戒すべきネガティブ情報**は `!!...!!` で囲む（赤太字で表示）。ポジティブな話題には使わない。
- 改行：長文は `\\n` で改行を入れて、1文を 40〜60字程度に収める。読みやすい呼吸を作る。
- 装飾は控えめに（**太字** 5〜10箇所、==下線== 2〜4箇所、!!注意!! 0〜2箇所）。多用すると逆に読みにくい。

【!! 注意マークアップの使用例】
- 著作権訴訟・侵害ニュースで「無断使用」「!!無断学習!!」のように
- 規制違反・罰金で「!!違反!!」「!!制裁金==1億ドル==!!」のように
- AI ハルシネーション・誤情報問題で「!!幻覚（誤情報）!!」のように
- セキュリティ脆弱性で「!!情報漏洩!!」「!!脆弱性!!」のように
- ※ 通常のニュースには無理に使わず、本当にリスク・警戒情報のときだけ

【書き方ルール（フィールド別）】
- title_ja：自然な日本語意訳。本文 25 字以内。中の重要キーワードを **太字** で囲む（1〜2箇所）。
  例：「**OpenAI**、AI スマホを **2027年** 発売へ」
- simple_explanation：小学校高学年でもわかる説明。専門用語は使わずたとえ話を活用。**太字**と==マーカー==を活用。
  100〜150字。改行を含めてOK。
- detail_summary：詳細セクションの先頭に出る濃い1文サマリー。**太字**と==マーカー==を活用。50〜90字。
...
- detail_points：詳細を構造化した箇条書き 4〜6 項目。各項目は `**見出し**：本文` 形式で書く。
  本文は 50〜100字、専門用語OK・**太字**と==マーカー==を活用。固有名詞・数値・年月などを積極的に入れて密度を上げる。
...
- detail_text：detail_points の後ろに来る深掘り段落（読者は「もっと詳しく知りたい人」のみが読む前提）。
  ...
  改行を入れて 2〜3 段落に。**太字**と==マーカー==も使ってOK。専門用語は補足を付けつつ、密度高めで書いて良い。
- points：トップに出る重要ポイント3つ。各20〜40字、**太字**と==マーカー==を活用。「何が」「どれだけ」「なぜ」が立つ短文。
- before / after：30〜60字。**太字**で要点強調OK。
- impact：あなた（読者）への影響を1文。40〜80字。**太字**と==マーカー==を活用。
- overview：今日全体の総括。120〜200字。**太字**で重要トピック名を強調。改行で2〜3文に。
- tip：title 15 字以内、content 80〜150字。content 内で **太字** と ==マーカー== を活用。実用的で具体的に。
...
{{
  ...
  "topics": [
    {{
      ...
      "simple_explanation": "**太字**と==マーカー==と改行を含む説明（100〜150字）",
      ...
      "detail_text": "深掘り段落（250〜400字）。背景・経緯・仕組み・展望などを **太字** や ==マーカー== を交えて 2〜3 段落で。",
      "points": ["**重要**な==数値==の含まれた項目1", "...", "..."],
  ...
  "tip": {{
    ...
    "content": "**太字**と==マーカー==を含む Tips 本文（80〜150字）"
  }}
}}
