...
- faq：そのトピックを読んだ「AI に詳しくない読者」がつまずきそうな疑問を **3〜7 個** 用意する Q&A 配列。
  数は記事の難易度に応じて柔軟に決める（やさしければ 3、専門用語が多ければ 7）。
  各 Q&A の構成：
    - q: 読者が抱きそうな素朴な疑問（10〜30字）。「○○ってなに？」「なぜ重要？」「私にどう関係ある？」など
    - a: 80〜150字のやさしい回答。**太字** を活用、改行も使ってOK。専門用語は使わずにたとえ話で説明
  内訳の目安：
    - 🔤 用語系（最低 2 個必須）：本文に出てきた専門用語・カタカナ語・略称を解説（例：「LLM ってなに？」）
    - 💡 概念系：「なぜこのニュースが重要？」「これまでとどう違う？」
    - 🌍 影響系：「私の生活にどう関係する？」「すぐに使える？」
...
{{
  ...
  "topics": [
    {{
      ...
      "faq": [
        {{ "q": "○○ってなに？（用語系）", "a": "**太字** を含む 80〜150字のやさしい説明" }},
        {{ "q": "なぜこれが重要？", "a": "..." }},
        {{ "q": "私の生活にどう関係する？", "a": "..." }}
      ],
